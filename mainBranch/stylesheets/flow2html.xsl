...
<!-- @version-tag-tools@ -->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:matml="http://www.matml.org/"
    xmlns:flow="http://lca.jrc.it/ILCD/Flow" xmlns:common="http://lca.jrc.it/ILCD/Common" xmlns:epd="http://www.iai.kit.edu/EPD/2013" exclude-result-prefixes="flow common xs">
    ...
    <xsl:template match="common:other/epd:isA">
        <xsl:call-template name="masterTemplate">
            <xsl:with-param name="fieldDescSuffix"><xsl:text>ist ein (</xsl:text><xsl:value-of select="@type"/>)</xsl:with-param>
    ...
    <xsl:template match="common:other/epd:referenceToVendor">
        <xsl:call-template name="masterTemplate">
            <xsl:with-param name="fieldDescSuffix">Hersteller</xsl:with-param>
            <xsl:with-param name="disableDescendantCount" select="true()"/>
            <xsl:with-param name="copy" select="true()"/>
            <xsl:with-param name="data">
                <xsl:call-template name="renderReference"/>
    ...
    <xsl:template match="common:other/epd:vendorSpecificProduct">
        <xsl:call-template name="masterTemplate">
            <xsl:with-param name="fieldDescSuffix">herstellerspezifisches Produkt</xsl:with-param>
            <xsl:with-param name="disableDescendantCount" select="true()"/>
            <xsl:with-param name="copy" select="true()"/>
            <xsl:with-param name="data">
                <xsl:choose>
    ...
    <xsl:template match="flow:LCIMethod">
        <xsl:apply-templates select="flow:typeOfDataSet"/>
        <xsl:apply-templates select="common:other/*"/>
    </xsl:template>

    <xsl:template match="matml:MatML_Doc">
        <xsl:call-template name="masterTemplate">
            <xsl:with-param name="fieldShortDesc">Materialeigenschaften</xsl:with-param>
            <xsl:with-param name="paddingleft">30</xsl:with-param>
        </xsl:call-template>
        <xsl:apply-templates select="matml:Material/matml:BulkDetails/matml:PropertyData"/>
    </xsl:template>

    <xsl:template match="matml:PropertyData">
        <xsl:variable name="property" select="@property"/>
        <xsl:call-template name="masterTemplate">
            <xsl:with-param name="fieldShortDesc">
                <xsl:value-of select="parent::*/parent::*/parent::*/matml:Metadata/matml:PropertyDetails[@id=$property]/matml:Name"/>
            </xsl:with-param>
            <xsl:with-param name="paddingleft">40</xsl:with-param>
            <xsl:with-param name="data">
                <xsl:value-of select="matml:Data/text()"/>
                <xsl:text> </xsl:text>
                <xsl:value-of select="parent::*/parent::*/parent::*/matml:Metadata/matml:PropertyDetails[@id=$property]/matml:Units/@name"/>
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="flow:name">
        ...
        <xsl:apply-templates select="parent::*/common:other/epd:*"/>
        <xsl:apply-templates select="parent::*/common:other/matml:*"/>
    </xsl:template>
        ...
        <xsl:call-template name="masterTemplate">
            ...
            <xsl:with-param name="data">
                <xsl:call-template name="renderReference">
                    ...
                    <xsl:with-param name="stdLinkText"
                        select="/flow:flowDataSet/flow:flowProperties/flow:flowProperty[@dataSetInternalID=$id]/flow:referenceToFlowPropertyDataSet/common:shortDescription[@xml:lang=$defaultLanguage]/text()"
                    />
                </xsl:call-template>
            </xsl:with-param>
        </xsl:call-template>
        ...
        <xsl:variable name="data">
            <xsl:choose>
                ...
                <xsl:when test="$currentName='meanValue'">
                    ...
                    <xsl:text> </xsl:text>
                    <xsl:variable name="flowPropertyId" select="./@dataSetInternalID"/>
